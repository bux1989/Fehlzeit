...
<script>
let React, ReactDOM, FehlzeitenPageWrapper;
...
export default {
  ...
  mounted() {
    this.loadReactAndMount();
  },
  beforeUnmount() {
    if (this.reactMounted && ReactDOM && this.$refs.reactContainer) {
  ...
  methods: {
    async loadReactAndMount() {
      try {
        ...
        if (!React) {
          ...
          const module = await import('./FehlzeitenPageWrapper.js');
          FehlzeitenPageWrapper = module.default;
        }
        ...
        this.$emit('trigger-event', {
          name: 'error',
          event: { message: 'Failed to load component: ' + error.message }
        });
      }
    },
    ...
    mountReactComponent() {
      ...
      ReactDOM.render(element, this.$refs.reactContainer);
      this.reactMounted = true;
    },
    
    updateReactComponent() {
      if (!this.reactMounted || !this.$refs.reactContainer || !FehlzeitenPageWrapper) return;
